...
export default class StickyEvents {
  ...
  fire(isSticky, stickyTarget, position) {
    ...
    stickyTarget.dispatchEvent(
      new CustomEvent(isSticky ? StickyEvents.STUCK : StickyEvents.UNSTUCK, {
        bubbles: true,
        detail: { isSticky, position },
      }),
    );
  ...
  getSentinelPosition(stickyElement, sentinel, className) {
    ...
    switch (className) {
      ...
      case ClassName.SENTINEL_BOTTOM: {
        const parentPadding = Number.parseInt(parentStyle.paddingTop);
  ...
  isSticking(stickyElement) {
    ...
    const topSentinelTopPosition = Math.abs(
      Number.parseInt(
        window.getComputedStyle(topSentinel).getPropertyValue('top'),
      ),
    );
